 ...
 * @param inFormat Collector identifier describing the input format
...
object EnrichJobConfig {
  ...
  def loadConfigFrom(
    args: Array[String]
  ): ValidatedNelMessage[(ParsedEnrichJobConfig, List[(URI, String)])] = {
        ...
        (resolver |@| registry |@| loader) { (res, reg, l) =>
          (
            ParsedEnrichJobConfig(c.inFolder, c.inFormat, c.outFolder, c.badFolder,
              res, reg, l, new DateTime(c.etlTstamp)),
            filesToCache(registry)
          )
        }
   ...
   */
  private def filesToCache(registry: ValidatedNelMessage[EnrichmentRegistry]): List[(URI, String)] =
    registry match {
      case Success(reg) =>
